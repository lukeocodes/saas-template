...
export type MiddlewareConfig = Record<
	string,
	MiddlewareFunction | CustomMiddleware | MiddlewareFunction[] | CustomMiddleware[]
>;
...
export default function createMiddleware(
	...
	return async (request: NextRequest): Promise<NextResponse> => {
		...
		const beforeResult = await executeGlobalMiddleware("before", request, globalMiddleware);
		if (beforeResult) {
		...
		for (const [key, middlewareFunctions] of Object.entries(pathMiddlewareMap)) {
			if (matchesPath(key, path)) {
				response = await executePathMiddleware(request, middlewareFunctions, response);
			}
		}

		const afterResult = await executeGlobalMiddleware("after", request, globalMiddleware);
		if (afterResult) {
...
function handleMiddlewareRedirect(request: NextRequest, response: NextResponse): NextResponse {
...
function isRedirect(response: NextResponse | null): boolean {
	return Boolean(response && [301, 302, 303, 307, 308].includes(response.status));
}

function updateRequestWithResponse(request: NextRequest, response: NextResponse): NextRequest {
